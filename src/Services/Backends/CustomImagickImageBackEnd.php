...
namespace Athulr\LaravelQr\Services\Backends;
...
final class CustomImagickImageBackEnd implements ImageBackEndInterface
{
    /**
     * @var string
     */
    ...
    public function new(int $size, ColorInterface $backgroundColor) : void
    {
        ...
        // Determine sampling factor based on size to balance quality and memory
        if ($size <= 500) {
            $this->samplingFactor = 4;
        } elseif ($size <= 1000) {
            $this->samplingFactor = 2;
        } else {
            $this->samplingFactor = 1; // Native resolution for huge images
        }

        $internalSize = $size * $this->samplingFactor;

        $this->image = new Imagick();
        $this->image->newImage($internalSize, $internalSize, $this->getColorPixel($backgroundColor));
    ...
    public function scale(float $size) : void
    {
        ...
        $scaledSize = $size * $this->samplingFactor;
        
        $this->draw->scale($scaledSize, $scaledSize);
        $this->matrices[$this->matrixIndex] = $this->matrices[$this->matrixIndex]
            ->multiply(TransformationMatrix::scale($scaledSize));
    }

    // ... translate and rotate rely on draw state, which is already scaled?
    // Wait, draw->translate works in current coordinate system. 
    // If we scaled the coordinate system by `scaledSize` (pixels per module),
    // then translating by 1 unit moves `scaledSize` pixels.
    // So translate/rotate methods do NOT need modification, strictly passing the module units.
    // However, I need to check if matrix multiplication needs adjustment.
    
    // The original implementation: matrices track the coordinate transform.
    // scale() updates the matrix with `TransformationMatrix::scale($size)`.
    // If I scale by `$scaledSize`, the matrix tracks that.
    // translate() multiplies by `TransformationMatrix::translate($x, $y)`.
    // It should be fine.
    ...
    // ... push, pop, drawPathWithColor, drawPathWithGradient unmodified ...
    ...
    public function done() : string
    {
        ...
        if ($this->samplingFactor > 1) {
            $this->image->resizeImage($this->targetSize, $this->targetSize, \Imagick::FILTER_LANCZOS, 1);
        }
